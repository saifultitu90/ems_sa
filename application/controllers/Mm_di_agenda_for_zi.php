...
class Mm_di_agenda_for_zi extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Mm_di_agenda_for_zi');
        $this->locations=User_helper::get_locations();
        $this->controller_url='mm_di_agenda_for_zi';
    }
...
//          $data['c_items']=Query_helper::get_info($this->config->item('table_mm_di_sales_target_bm'),'*',array('agenda_id ='.$group_id));
    ...
    public function system_forward()
    {
        ...
        {
            $this->db->select('*');
            $this->db->from($this->config->item('table_mm_di_meeting_forward'));
            $this->db->where('agenda_id',$agenda_id);
            $this->db->where('division_id',$division_id);
            $mdata['meeting_complete']=$this->db->get()->row_array();
            if($mdata['meeting_complete'])
            {
                $data['forward_status']=$this->config->item('system_status_forwarded_to_di');
                $data['date_updated']=$time;
                $data['user_updated'] = $user->user_id;
                $this->db->where('agenda_id',$agenda_id);
                $this->db->where('division_id',$division_id);
                $this->db->update($this->config->item('table_mm_di_meeting_forward'), $data);
            }else
            {
                $data['agenda_id']=$agenda_id;
                ...
                $data['date_created']=$time;
                $data['user_created'] = $user->user_id;
                Query_helper::add($this->config->item('table_mm_di_meeting_forward'),$data);
            }
        }else
        {
            $this->db->select('*');
